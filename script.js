...
(function faqAnimateJs() {
  ...
  const DURATION = prefersReduce ? 0 : 220;
  ...
  detailsList.forEach((el) => {
    ...
    function expand() {
      stopAnim();
      // Open but take content out of flow to measure without pushing layout
      ...
      const target = content.scrollHeight; // includes padding
      // Restore back and start from 0 height
    ...
    function collapse() {
      stopAnim();
      const start = content.getBoundingClientRect().height || content.scrollHeight;
      if (!start) {
  ...
  detailsList.forEach((el) => {
    ...
    function expand() {
      ...
      requestAnimationFrame(() => {
        content.style.transition = `height ${DURATION}ms ${EASE}, opacity 200ms ease`;
    ...
    function collapse() {
      if (animating) return;
      animating = true;
      // Lock the viewport relative to the collapsing item to avoid final nudge
      ...
      const onEnd = (ev) => {
        ...
        el.removeAttribute("open");
        finish();
        // Adjust scroll so the summary stays visually in place
        const newTop = el.getBoundingClientRect().top;
        const delta = newTop - anchorTop;
        // Ignore sub-pixel differences to avoid visible micro-jumps
        if (Math.abs(delta) > 1) {
    ...
    function onToggle(e) {
      ...
      if (typeof e.stopImmediatePropagation === "function")
        e.stopImmediatePropagation();
      // Avoid browser trying to keep focused <summary> anchored (can cause slight page nudge)
      try {
        summary.blur();
      } catch {}
      if (el.hasAttribute("open")) collapse();
...
(function smoothWheelScroll() {
  // Disabled by request: use native scrolling
  return;
  const prefersReduce =
    window.matchMedia &&
    window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  ...
  const MAX_STEP = 120; // cap per event after normalization
  const SPEED = 1.0; // multiplier from wheel delta to pixels
  const EASE = 0.36; // higher = быстрее реагирует, короче шлейф
  const KICK = 0.35; // мгновенный шаг сразу при событии
  const STOP_EPS = 0.2;
